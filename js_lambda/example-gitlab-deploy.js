function deploy(args, ctx) {
    ...
    try {
        var payload = {
            name:        "docs-review-" + short_sha,
            description: "documentation site deploy by gitlab, commit " + short_sha,
            properties: {
                provider: {
                    id: tgt_provider.id
                },
    ...
    var new_apiendpoint = create_apiendpoint(tgt_org, tgt_api, {
        ...
        properties: {
            ...
            resource: "/docs-review-" + short_sha + "/"
        }
    });
    ...
    var gitlab_env = find_gitlab_environment(gitlab_url, gitlab_token, "review-" + full_sha);
    if ( gitlab_env ) {
...
function stop(args, ctx) {
    ...
    var short_sha = args.substr(0,8);

    log("Will delete deployment associated with commit " + short_sha);

    var tgt_cntr = find_container_by_name(tgt_org, tgt_env, "docs-review-" + short_sha);

    if ( tgt_cntr ) {
        log("Listing endpoints for container");
        var endpoints = list_container_apiendpoints(tgt_org, tgt_cntr);
        for each (e in endpoints) {
            log("deleting endpoint " + e.id);
